...
public:
    ...
    GameObject(x, y, SIZE, SIZE, TAG_ENEMY)
  {
    ...
    const float x2 = x + SIZE * 4, y2 = y;
    const float x3 = x, y3 = y + SIZE * 3;

    std::shared_ptr<StateComponent::State> move1State = std::make_shared<MoveState>(speed, x1, y1);
    std::shared_ptr<StateComponent::State> move2State = std::make_shared<MoveState>(speed, x2, y2);
    std::shared_ptr<StateComponent::State> move3State = std::make_shared<MoveState>(speed, x3, y3);

    std::shared_ptr<StateComponent> sc = std::make_shared<StateComponent>(*this);
    sc->setStartState(move1State);
    sc->addTransition(move1State, move2State, std::make_shared<PointProximityTransition>(x1, y1, SIZE/8));
    sc->addTransition(move2State, move3State, std::make_shared<PointProximityTransition>(x2, y2, SIZE/8));
    sc->addTransition(move3State, move1State, std::make_shared<PointProximityTransition>(x3, y3, SIZE/8));
