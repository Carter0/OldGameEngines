...
MoveAcrossState::MoveAcrossState(float speedX, float speedY): m_speedX(speedX), m_speedY(speedY) {}
void MoveAcrossState::update(GameObject & gameObject, Level & level) {
	gameObject.physicsComponent()->setVx(m_speedX);
	gameObject.physicsComponent()->setVy(m_speedY);
}


void StationaryState::update(GameObject & gameObject, Level & level) {
	gameObject.physicsComponent()->setVx(0);
	gameObject.physicsComponent()->setVy(0);
}
...
{
	mStep = 0;
	//std::cout << mStep << mSteps << std::endl;
}
