...
Vector2D GameObject::m_cameraOffset = Vector2D(0,0);

//! \brief A class to represent a generic game object.
GameObject::GameObject(float x, float y, float w, float h, int tag) :
		mX(x), mY(y), mW(w), mH(h), mTag(tag) {
}
...
void GameObject::onDestroy(Level &level) {
	for (auto genericComponent : mGenericComponents) {
		genericComponent->onDestroy(level);
	}
}
