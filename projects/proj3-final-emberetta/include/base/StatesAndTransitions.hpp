...
/**
 * Enemy will move in the direction specificed
 */
class MoveAcrossState: public StateComponent::State {
public:

	MoveAcrossState(float speedX, float speedY);
	virtual void update(GameObject & gameObject, Level & level) override;

private:

	const float m_speedX, m_speedY;

};

//! \brief Just Vibin'.
class StationaryState: public StateComponent::State {
public:
