...
Coin::Coin(float x, float y, float w, float h): GameObject(x, y, w, h, TAG_COIN) {
	setPhysicsCompenent(std::make_shared<PhysicsComponent>(*this, false));
	SDL_Rect src{0, 0, 563, 564};
    setRenderCompenent(std::make_shared<TextureRenderComponent>(*this, "Gold_1.png", src));
}

std::ostream& Coin::print(std::ostream &out) {
	return out << TAG_COIN << " " << this->x() << " " << this->y() << " " << this->w() << " " << this->h() << std::endl;
}
